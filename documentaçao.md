```
const int8_t TRIG_PIN = 32;
const int8_t ECHO_PIN = 34;

#define LIMIAR_NOITE 500
#define LIMIAR_DISTANCIA 30

#define TEMPO_AMARELO_NOITE 500
#define TEMPO_VERDE_AVENIDA_MAX 4000
#define TEMPO_VERDE_AVENIDA_MIN 2000
#define TEMPO_VERDE_PEDESTRE 8000
#define TEMPO_AMARELO 2000
#define TEMPO_VERMELHO_RUA_MAX 4000
...
private:
  ...
  void setLuz1(int V, int A, int G, int tempoAtual) {
    digitalWrite(vermelho1Pin, V);
    digitalWrite(amarelo1Pin, A);
    digitalWrite(verde1Pin, G);
  ...
  void setTrafficLightYellowBlinking(unsigned long tempoAtual) {
    if (tempoAtual - tempoPisca >= TEMPO_AMARELO_NOITE) {
      ...
      digitalWrite(verde2Pin, estado ? HIGH : LOW);
...
public:
  ...
  void begin() {
    ...
    setLuz1(LOW, LOW, HIGH, millis());
    setLuz2(HIGH, LOW, LOW, millis());
  ...
  void update() {
    ...
    bool Vermelho1Ativo = digitalRead(vermelho1Pin) == HIGH;
    bool Verde1Ativo = digitalRead(verde1Pin) == HIGH;
    bool Amarelo1Ativo = digitalRead(amarelo1Pin) == HIGH;
    bool Vermelho2Ativo = digitalRead(vermelho2Pin) == HIGH;
    bool Verde2Ativo = digitalRead(amarelo2Pin) == HIGH;
    bool Amarelo2Ativo = digitalRead(verde2Pin) == HIGH;
    
    
    // LÓGICA DO SEMÁFORO 1 (CARROS - AVENIDA)
    
    if (Verde1Ativo) {
      long tempoMinimo = TEMPO_VERDE_AVENIDA_MAX;
      if (deteccaoRua2) {
          tempoMinimo = TEMPO_VERDE_AVENIDA_MIN;
      }
      
      if (tempoAtual - ultimoTempo1 >= tempoMinimo) {
        setLuz1(LOW, HIGH, LOW, tempoAtual);
      }
      
    } else if (Amarelo1Ativo) {
      if (tempoAtual - ultimoTempo1 >= TEMPO_AMARELO) {
        setLuz1(HIGH, LOW, LOW, tempoAtual);
      }
      
    } else if (Vermelho1Ativo) {
        if (Vermelho2Ativo && (tempoAtual - ultimoTempo1 >= TEMPO_AMARELO)) {
            setLuz1(LOW, LOW, HIGH, tempoAtual);
        }
    }


    // LÓGICA DO SEMÁFORO 2 (PEDESTRES)
    
    if (Verde2Ativo) {
      if (tempoAtual - ultimoTempo2 >= TEMPO_VERDE_PEDESTRE) {
        setLuz2(LOW, HIGH, LOW, tempoAtual);
      }
      
    } else if (Amarelo2Ativo) {
      if (tempoAtual - ultimoTempo2 >= TEMPO_AMARELO) {
        setLuz2(HIGH, LOW, LOW, tempoAtual);
      }
      
    } else if (Vermelho2Ativo) {
      if (Vermelho1Ativo) {
        if (tempoAtual - ultimoTempo2 >= TEMPO_VERMELHO_RUA_MAX || deteccaoRua2) {
          setLuz2(LOW, LOW, HIGH, tempoAtual);
        }
      }
    }
